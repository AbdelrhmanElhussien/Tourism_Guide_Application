...
class HotelService {
  ...
  Future<HotelModel> fetchHotelDetails(String id) async {
    try {
      final response = await _dio.get('${ApiConstant.hotelsEndPoint}/$id');
      ...
      if (response.statusCode == 200) {
        ...
        if (data is Map<String, dynamic>) {
          if (data['success'] == true && data['data'] != null) {
            return HotelModel.fromJson(data['data'] as Map<String, dynamic>);
          }
      ...
      throw Exception('Failed to load hotel details (Status: ${response.statusCode})');
    } on DioException catch (e) {
      final errorData = e.response?.data;
      String message = 'Something went wrong';
